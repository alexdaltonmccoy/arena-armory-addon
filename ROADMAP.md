...
- **Android production launch submitted, 2026-09-18 (wow-classic-armory / Arena Armory mobile app) — the 9/15 production-access application (below) was granted; Alex forwarded Google's "Congratulations" email and this session took it from grant to a live submission.** Checked Play Console directly rather than assume the grant meant anything was already live: Production track showed "Inactive," zero releases. Used "Add from library" to promote **versionCode 15 (1.1.0)** — the exact build already live and real-device-smoke-tested on the alpha closed-testing track since Aug 19 — rather than cutting a fresh EAS build, since it was already the most-verified artifact available. Selected worldwide availability (all 176 countries + rest of world) and 100% rollout with no staged percentage, both explicit calls from Alex rather than assumed defaults. One real blocker hit and cleared before submission: Play's review step refused the release with "No countries or regions have been selected for this track" until country availability was added — a required gate, not optional. Submitted "2 changes for review" (the release + country availability) via Publishing overview (managed publishing is off for this app, so nothing auto-publishes without this explicit step); Google's stated window is 7 days or less. Nothing further required on our end until the decision email — once live, the MVP-honesty framing in `PORTFOLIO_ROADMAP.md` (currently "Web + iOS; Android coming soon") needs updating to include Android.
